 ...
 * limits and the last part the lower limit for the initializer write,
...
std::vector<const llvm::GenericValue *> WBCoherenceCalculator::getTrackedCoherenceLocs()
{
	std::vector<const llvm::GenericValue *> addrs;
...
std::pair<int, int>
WBCoherenceCalculator::getPossiblePlacings(const llvm::GenericValue *addr, Event store, bool isRMW)
{
					...
					const std::vector<Event> &ls) const
{
	 ...
	 */
	if (sLab->getPos() == g.getLastThreadEvent(sLab->getThread()) && sLab->getReadersList().empty() &&
...
std::vector<Event>
WBCoherenceCalculator::getCoherentRevisits(const WriteLabel *sLab, std::vector<Event> &&ls)
{
	const auto &g = getGraph();
...
std::vector<Event> WBCoherenceCalculator::getPrefixRestricted(const WriteLabel *wLab, const VectorClock &view) const
{
	std::vector<Event> prefix;

	auto &locMO = stores_.at(wLab->getAddr());
	auto it = std::find(locMO.begin(), locMO.end(), wLab->getPos());
	BUG_ON(it == locMO.end());

	return prefix;
}

int WBCoherenceCalculator::getStoreOffset(const llvm::GenericValue *addr, Event store) const
{
	// WB calculator does not track concrete positions;
	// check that the store is in this location's MO just in case
	auto &locMO = stores_.at(addr);
	auto it = std::find(locMO.begin(), locMO.end(), store);
	BUG_ON(!store.isBottom() && !store.isInitializer() && it == locMO.end());
	return -1;
}
