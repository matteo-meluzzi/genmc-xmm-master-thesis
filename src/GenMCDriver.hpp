...
public:
	...
	struct Result {
		...
		unsigned lbRacesExplored{}; /* Number of load buffering races explored */ 
		...
		auto operator+=(const Result &other) -> Result& {
			...
			lbRacesExplored += other.lbRacesExplored;
#ifdef ENABLE_GENMC_DEBUG
	...
	struct Execution {
		...
		/* 
		(value, variable) -> {ReadLabel} 
		A map from a pair of a variable (SAddr) and a value to a set of ReadLabel.
		This map is needed to keep track of which reads in the graph read from a certain variable and a certain value.
		The same could be achieved by looping through the execution graph, but using a map is more efficient.
		*/
		ValAddrTidToReadsMap val_addr_to_reads_in_graph; 
		/*
		read -> value
		A map from a read label to a value.
		This map is used to keep track of which value a read should read after we drop some events during graph restriciton.
		When we do re-execution that read should read from a write that has the same value as the one stored in this map.
		*/
		...
		// constructor for a XMM execution
        Execution(std::unique_ptr<ExecutionGraph> g, 
				  LocalQueueT &&w, 
				...
				ReadToValMap read_in_graph_to_val,
				std::unordered_set<Event, EventHasher> committedEvents)
            : graph(std::move(g)), workqueue(std::move(w)), isXMM(true), racyRead(std::move(rLab)), racyWrite(std::move(wLab)), val_addr_to_reads_in_graph(std::move(val_addr_to_reads_in_graph)), read_in_graph_to_val(std::move(read_in_graph_to_val)), committedEvents(std::move(committedEvents)) {}
...
// GenMC-XMM methods
	...
	/* Construct XMM execution from graph g an new graph og. Metadata computed based on difference between original graph g and restricted graph og. */
	Execution constructXMMExecution(const ExecutionGraph &g, std::unique_ptr<ExecutionGraph> og, Event revisitPos, std::unique_ptr<ReadLabel> racyRead, std::unique_ptr<WriteLabel> racyWrite);

	// Revisit Methods
	/* XMM: called when a LBRaceXMMRevisit is popped from the worklist. Pushes an XMM execution to the stack. */
    bool lbRaceXMMRevisit(std::unique_ptr<LBRaceXMMRevisit> lbRev);
	/* XMM: called when a BackwardXMMRevisit is popped from the worklist. Pushes an XMM execution to the stack. */
	bool backwardXMMRevisit(std::unique_ptr<BackwardXMMRevisit> brXmm);
	/* XMM: called when a ReadForwardXMMRevisit is popped from the worklist. Pushes an XMM execution to the stack. */
	bool forwardXMMRevisit(std::unique_ptr<ReadForwardXMMRevisit> fr);
	/* XMM: called when a ReadForwardXMMRevisit is popped from the worklist. Pushes an XMM graph to the stack */
	bool forwardRacyReadTidXMMRevisit(std::unique_ptr<ReadForwardRacyReadTidXMMRevisit> fr);
	/* XMM: revisits events that are not in the porf prefix of the LB racy read */
    bool hbPredsXMMRevisit(std::unique_ptr<HbPredsXMMRevisit> hbRev);
	/* XMM: revisits a committed write and a committed read. This is very rare but it can happen when multiple writes can be matched to a R_bottom. */
	bool committedWriteXMMRevisit(std::unique_ptr<CommittedWriteXMMRevisit> rev);

	// Consistency methods
	/* is eLab1 before eLab2 in the eco relation? */
	bool isEcoBefore(const EventLabel *eLab1, const EventLabel *eLab2) const;
	/* XMM: returns true if the uncommitted reads are stable. (Definition 10). */
	bool areXMMUncommittedReadsStable(const ExecutionGraph &g) const;
	/* XMM: is current execution graph consistent? */
	bool xmmExecutionConsistent() const;

	// Resvisit calculation methods
	/* XMM: calculates backward revisits for wLab in an XMM graph */
	bool calcXMMBackwardRevisits(const WriteLabel *wLab);
    /* XMM: searches for load buffering races and adds additional causally cyclic XMM revisits to the work queue */
    void calcXMMRevisitsLbRace();
	/* XMM: drop everything except cycle and its porf prefix, and re-execute */
	void calcXMMRevisitsHbPredecessors();
	/* XMM: revisits the reads in the thread of the LB racy read */
	void calcXMMRevisitsLBRacyReadThread();

	// Helper methods
	/* XMM: is g a duplicate? if not memorize (add) it */
	bool isDupElseAdd(const ExecutionGraph &g);
    /* XMM: checks if a pair of events forms a load buffering race  */
    bool areInLoadBufferingRace(const ReadLabel *rLab, const WriteLabel *wLab) const;
    /* XMM: calculate which events should be kept given that racyRead and racyWrite are in a LB race */
    View calcLBRaceKeptEventsXMM(const ExecutionGraph &g, const ReadLabel *racyRead) const;
	/* XMM: adds all events in the graph in a hash set and returns it. Used for calculating committed events. */
	std::unordered_set<Event, EventHasher> addAllEventsToSet(const ExecutionGraph &g) const;
	...
	std::optional<std::ofstream> outputDotGraphFileStream;
	void dotPrintGraph(std::ofstream &ss);
	...
	std::unordered_set<const ExecutionGraph *, ExecutionGraphPtrHasher, ExecutionGraphPtrEqual> visitedExecs;
};
...
void executeMDPrint(const EventLabel *lab,
