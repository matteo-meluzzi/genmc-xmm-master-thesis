...
private:
	...
	struct StackItem {
		...
		StackItem(StackItemType t, Event e, Event shouldRf,
				  ...
				  PromiseSet &&promises, int newMoPos)
		...
		/* Appropriate positions for the writes in the prefix
		 * to be restored */
		std::vector<std::pair<Event, Event> > moPlacings;

		/* The set of promised writes that need to be certified. */
		PromiseSet promises;
...
public:
	...
	// TODO: I made it virtual for WKMODriver, but perhaps we need a better approach
	virtual bool scheduleNext();
...
protected:
	...
	virtual bool ensureConsistentStore(const WriteLabel *wLab);
	...
	std::vector<Event> properlyOrderStores(const ReadLabel* rLab, std::vector<Event> &stores);

	/* Returns true if the current graph is consistent */
	bool isConsistent(bool checkFull);

	/* Returns true if the current graph is consistent.
	 * Skips the consistency checks if they are disabled at given program point */
	...
	/* Creates forward revisit item */
	virtual std::unique_ptr<FRevItem> createForwardRevisit(Event r, Event w);

	/* Creates backward revisit item */
	//  TODO: Make more generic virtual function that creates `RevItem`?
	virtual std::unique_ptr<BRevItem> createBackwardRevisit(Event r, Event w);

	/* Creates mo revisit item */
	virtual std::unique_ptr<MOItem> createMORevisit(Event p, int moPos);

	/* Checks whether equivalent backward revisit was already added to the worklist */
	virtual bool isDuplicateRevisit(const BRevItem* bri);

	void restrictRevisitSet(const EventLabel *lab);

	// TODO: fixme!
	RevisitSet& getRevisitSet(unsigned int stamp);

	/* Calculates revisit options and pushes them to the worklist.
	 * Returns true if the current exploration should continue.
	 */
	bool calcRevisits(const WriteLabel *sLab);
	bool calcRevisits(const WriteLabel *sLab, std::vector<Event> &&loads);
	bool calcLibRevisits(const EventLabel *lab);

	/*** Coherence related ***/

	/* Returns the range of all the coherent places where a store can be inserted */
	virtual
	std::pair<int, int> getCoherentPlacings(const llvm::GenericValue *addr, Event store, bool isRMW);

	/*** Graph repairing ***/

	/* Repairs graph after the restriction. */
	virtual bool repairGraph(WorkItem* wi);
...
private:
	...
	/* Checks for load buffering pattern */
	void checkForLoadBufferingPattern();

    	/* Auxiliary function to check for (fai_rmw + rf)^+ path between read and write accesses
     	 * (accesses connected by such path should be excluded from the load buffering pattern) */
	bool checkForFaiRfPath(const ReadLabel* rLab, const WriteLabel* wLab);
	...
	/* Does some preliminary consistency checks before the fixpoint is over */
	virtual bool doPreConsChecks();
	...
	/* Number of executions with unfulfilled promises explored */
	int exploredUnfulfilled;
