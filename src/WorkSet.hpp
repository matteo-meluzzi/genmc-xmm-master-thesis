...
/*
 * FRevPrmItem class - Represents a forward revisit with promises
 */
class FRevPrmItem : public FRevItem {

public:
	FRevPrmItem(Event p, Event r, PromiseSet &&promises)
		: FRevItem(WI_FRevPrm, p, r),
		  promises(std::move(promises)) {}

	/* Returns (releases) promises */
	PromiseSet &&getPromisesRel() {
		return std::move(promises);
	}

	/* Returns (but does not release) the prefix of the revisiting event */
	const PromiseSet &getPromisesNoRel() const {
		return promises;
	}

	static bool classof(const WorkItem *item) {
		return item->getKind() == WI_FRevPrm;
	}
...
public:
	...
	/* Returns (but doest not release) the coherence placing in the prefix */
	const std::vector<std::pair<Event, Event> > &getMOPlacingsNoRel() const {
		return std::move(moPlacings);
	}
...
/*
 * BRevPrmItem class - Represents a po-rf cyclic backward revisit with promises
 */
class BRevPrmItem : public BRevItem {

public:
	BRevPrmItem(Event p, Event r,
		    std::vector<std::unique_ptr<EventLabel> > &&prefix,
		    std::vector<std::pair<Event, Event> > &&moPlacings,
		    PromiseSet &&promises)
		: BRevItem(WI_BRevPrm, p, r, std::move(prefix), std::move(moPlacings)),
		  promises(std::move(promises)) {}
	...
	/* Returns (but does not release) promises */
	const PromiseSet &getPromisesNoRel() const {
		return promises;
	}

	static bool classof(const WorkItem *item) {
		return item->getKind() == WI_BRevPrm;
	}
...
/*
 * MOPrmItem class - Represents an alternative MO position with promises for a store
 * (Used by drivers that track MO only)
 */
class MOPrmItem : public MOItem {

public:
	MOPrmItem(Event p, int moPos, PromiseSet &&promises)
		: MOItem(WI_MOPrm, p, moPos),
		  promises(std::move(promises)) {}
