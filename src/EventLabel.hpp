...
/* Justification Set */
typedef std::unordered_set<Event, EventHasher> JFSet;
...
protected:
	...
	EventLabel(EventLabelKind k, unsigned int s, llvm::AtomicOrdering o, Event p)
		: kind(k), stamp(s), ordering(o), position(p) {}
...
public:
	...
	/* Returns true if this label corresponds to a relaxed access */
...
private:
	...
	/* Additional causal writes, except those captured by `rfe;(po+rf)^*` (WKMODriver) */
	JFSet jfSet;
};
...
public:
	 ...
	 * TODO: make it private again? (discuss with Michalis) */
...
public:
	...
	/* Returns a list of the external reads reading from this write */
	std::vector<Event> getExternalReadersList() const {
		std::vector<Event> extReaders;
		for (auto &r: getReadersList()) {
			if (getThread() != r.thread) {
				extReaders.push_back(r);
			}
		}
		return extReaders;
	}
