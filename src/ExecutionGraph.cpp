...
bool ExecutionGraph::checkInGraph(Event e) const
{
...
EventLabel *ExecutionGraph::getPreviousLabel(const EventLabel *lab)
{
	return events[lab->getThread()][lab->getIndex() - 1].get();
}

const EventLabel *ExecutionGraph::getNextLabel(Event e) const
{
	return events[e.thread][e.index + 1].get();
}

const EventLabel *ExecutionGraph::getNextLabel(const EventLabel *lab) const
{
	return events[lab->getThread()][lab->getIndex() + 1].get();
}

EventLabel *ExecutionGraph::getNextLabel(Event e)
{
	return events[e.thread][e.index + 1].get();
}

EventLabel* ExecutionGraph::getNextLabel(const EventLabel *lab)
{
	return events[lab->getThread()][lab->getIndex() + 1].get();
}
...
const EventLabel *ExecutionGraph::getFirstThreadLabel(int tid) const
{
	// TODO: should I use 0 or another id?
	return events[tid][0].get();
}

Event ExecutionGraph::getFirstThreadEvent(int tid) const
{
	// TODO: should I use 0 or another id?
	return Event(tid, 0);
}
...
Event ExecutionGraph::getLastThreadAcquireAtLoc(Event upperLimit, const llvm::GenericValue *addr) const
{
	for (int i = upperLimit.index - 1; i > 0; i--) {
		...
		if (auto *fLab = llvm::dyn_cast<FenceLabel>(lab)) {
			if (fLab->isAtLeastAcquire())
				return Event(upperLimit.thread, i);
		}
		if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
			if (rLab->isAtLeastAcquire() && rLab->getAddr() == addr)
				return Event(upperLimit.thread, i);
		}
	}
...
bool ExecutionGraph::isRevisitable(const ReadLabel *rLab, const WriteLabel *sLab) const
{
	if (rLab->getAddr() != sLab->getAddr()) {
		return false;
	}

	// do not revisit reads in the same thread
	// because it would obviously violate coherence
	if (rLab->getThread() == sLab->getThread()) {
		return false;
	}

	auto &porf = getPorfBefore(sLab->getPos());

	auto &sHb = getHbBefore(sLab->getPos());
	auto &rHb = getHbBefore(rLab->getPos());

	bool isPorfBefore = porf.contains(rLab->getPos());
	bool isRacy = !sHb.contains(rLab->getPos()) && !rHb.contains(sLab->getPos());

	if (isPorfBefore) {
		// revisit only racy po-rf ancestors
		// TODO: is the check for Release/Acquire here is general enough
		//   to prevent various coherence violations?
		return isRacy && !(sLab->isAtLeastRelease() && rLab->isAtLeastAcquire());
	}

	// finally, return true if the read is not po-rf before the write and is revisitable
	return rLab->isRevisitable();

}

std::vector<Event> ExecutionGraph::getRevisitable(const WriteLabel *sLab) const
{
	return getRevisitable(sLab->getAddr(), getPorfBefore(sLab->getPos()));
}

std::vector<Event> ExecutionGraph::getRevisitable(const llvm::GenericValue *addr, const View &before) const
{
	...
	for (auto i = 0u; i < getNumThreads(); i++) {
		for (auto j = before[i] + 1u; j < getThreadSize(i); j++) {
			const EventLabel *lab = getEventLabel(Event(i, j));
			if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
				if (addr == rLab->getAddr() && rLab->isRevisitable())
...
std::pair<int, int>
ExecutionGraph::getCoherentPlacings(const llvm::GenericValue *addr, Event pos, bool isRMW)
{
...
std::vector<Event>
ExecutionGraph::getCoherentRevisits(const WriteLabel *wLab, std::vector<Event> &&ls)
{
	return getCoherenceCalculator()->getCoherentRevisits(wLab, std::move(ls));
}
...
ExecutionGraph::getPrefixLabelsNotBefore(const EventLabel *sLab, const ReadLabel *rLab) const
{
	return getPrefixLabelsNotBefore(sLab->getPorfView(), rLab);
}

std::vector<std::unique_ptr<EventLabel>>
ExecutionGraph::getPrefixLabelsNotBefore(const VectorClock &prefix, const ReadLabel *rLab) const
{
	std::vector<std::unique_ptr<EventLabel> > result;

	auto rTid = rLab->getThread();
	auto rIdx = rLab->getIndex();

	auto viewPtr = std::unique_ptr<VectorClock>(prefix.clone());
	auto &view = *viewPtr;

	if (isOnPorfCycle(rLab))
		view.update(rLab->getPorfView());

	auto before = getViewFromStamp(rLab->getStamp());
	for (auto i = 0u; i < getNumThreads(); i++) {
		for (auto j = before[i] + 1; j <= view[i]; j++) {
			// skip po ancestors of the read
			if (i == rTid && j > rIdx) {
				continue;
			}
			...
			if (auto *wLab = llvm::dyn_cast<WriteLabel>(curLab.get())) {
				wLab->removeReader([&](Event r) {
						return (!view.contains(r) && !before.contains(r)) ||
								(r.thread == rTid && r.index > rIdx);
					});
			} else if (auto *eLab = llvm::dyn_cast<ThreadFinishLabel>(curLab.get())) {
				if (!view.contains(eLab->getParentJoin()) &&
	...
	// When revisiting a read on po-rf cycle,
	// we also need to save labels of reads that
	// (a) belong to the same po-rf cycle as revisited read,
	// (b) have a stamp lesser than revisited read.
	// We save these labels to save their rf source write,
	// as it was at the point when revisit is generated.
	// This is because later it can be changed to some equivalent write
	// (the value of reads should not change though).
	if (isOnPorfCycle(rLab)) {
		const auto& porf = rLab->getPorfView();
		for (auto i = 0u; i < getNumThreads(); i++) {
			for (auto j = 0u; j <= porf[i]; j++) {
				Event e = Event(i, j);
				if (e == rLab->getPos() || !before.contains(e))
					continue;
				const EventLabel *curLab = getEventLabel(e);
				// by checking that `e` is porf "before" `r`,
				// we check that both events belong to the same porf cycle
				if (!curLab->getPorfView().contains(rLab->getPos()))
					continue;
				if (auto *rCurLab = llvm::dyn_cast<ReadLabel>(curLab)) {
					result.push_back(std::unique_ptr<EventLabel>(rCurLab->clone()));
				}
			}
		}
	}
...
bool ExecutionGraph::isOnPorfCycle(const EventLabel *lab) const
{
	return lab->getPorfView()[lab->getThread()] > lab->getIndex();
}
					...
					std::vector<std::pair<Event, Event> > &moPlacings)
{
	...
	for (const auto &e : inserted) {
		...
		if (auto *curRLab = llvm::dyn_cast<ReadLabel>(curLab)) {
			...
			if (curRf.index >= 0 && curRf.index < (int) getThreadSize(curRf.thread)) {
				auto curRfLab = events[curRf.thread][curRf.index].get();
				if (auto *wLab = llvm::dyn_cast<WriteLabel>(curRfLab)) {
...
//			auto *lab = events[i][j].get();
//			JFSet jfSet = lab->getJFSet();
//			JFSet newJfSet;
//			for (auto ce : jfSet) {
//				if (checkInGraph(ce)) {
//					newJfSet.insert(ce);
//				}
//			}
//			lab->setJFSet(std::move(newJfSet));
		}
	}
}
...
llvm::raw_ostream& operator<<(llvm::raw_ostream &s, const ExecutionGraph &g)
{
	...
	for (auto i = 0u; i < g.getNumThreads(); i++)
		s << g.events[i].size() << " ";
	s << "\n";
	if (auto *cc = llvm::dyn_cast<MOCoherenceCalculator>(g.getCoherenceCalculator())) {
		auto locs = cc->getTrackedCoherenceLocs();
		std::sort(locs.begin(), locs.end());
		for (auto loc : locs) {
			s << "Coherence at " << loc << ":\n";
			for (auto w : cc->getStoresToLoc(loc)) {
				s << "\t" << w << "\n";
			}
		}
	}
