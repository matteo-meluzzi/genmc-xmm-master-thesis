...
llvm::raw_ostream& operator<<(llvm::raw_ostream &s, GraphKind kind) {
	switch (kind)
	{
	case GraphKind::GenMC:
		return s << "GenMC";
	case GraphKind::XMM:
		return s << "XMM";
	case GraphKind::XMM_HB_PREDS:
		return s << "XMM HB PREDS";
	default:
		return s << "UNKNOWN GRAPH KIND";
	}
}
...
bool ExecutionGraph::isStoreReadBySettledRMW(Event store, SAddr ptr, const VectorClock &prefix) const
{
	for (const auto &lab : labels(*this)) {
		...
		auto *rLab = llvm::dyn_cast<ReadLabel>(&lab);
		if (!rLab->getRf() || rLab->getRf()->getPos() != store || rLab->getAddr() != ptr)
			continue;
...
llvm::raw_ostream& operator<<(llvm::raw_ostream &s, const ExecutionGraph &g)
{
	for (auto i = 0u; i < g.getNumThreads(); i++) {
		...
		for (auto j = 0u; j < g.getThreadSize(i); j++) {
			...
			// printing the stamp causes duplicate executions to be output because
			// graphs are hashed using this operator<< (converted to string)
			s << "\t" /* << lab->getStamp() << " @ "*/ << *lab << "\n";
		}
	}
	s << "Thread sizes:\n\t";
	...
	std::vector<SAddr> keys;
	for (auto &coh_info : g.coherence) {
		keys.push_back(coh_info.first);
	}
	std::sort(keys.begin(), keys.end());
	for (auto key : keys) {
		s << key << ": ";
		for (auto sIt = g.co_begin(key); sIt != g.co_end(key); ++sIt)
			s << sIt->getPos() << " ";
		s << "\n";
	}
	return s;
...
template <class T>
inline void ExecutionGraphPtrHasher::hash_combine(std::size_t& seed, const T& v)	const {
	seed ^= std::hash<T>()(v) + 0x9e3779b9 + (seed<<6) + (seed>>2);
}

std::size_t ExecutionGraphPtrHasher::operator()(const ExecutionGraph *graph) const noexcept {
	const ExecutionGraph &g = *graph;
	std::size_t hash = 0;
	for (auto tid = 0; tid < g.getNumThreads(); tid++) {
		for (auto index = 0; index < g.getThreadSize(tid); index++) {
			auto *eLab = g.getEventLabel({tid, index});
			hash_combine(hash, eLab->getKind());
			if (auto *rLab = llvm::dyn_cast<ReadLabel>(eLab)) {
				hash_combine(hash, rLab->getAddr().get());
				if (rLab->getRf()) {
					hash_combine(hash, rLab->getRf()->getThread());
					hash_combine(hash, rLab->getRf()->getIndex());
				} else {
					hash_combine(hash, 1);
				}
			}
			else if (auto *wLab = llvm::dyn_cast<WriteLabel>(eLab)) {
				hash_combine(hash, wLab->getAddr().get());
				hash_combine(hash, wLab->getVal().get());
			}
		}
	}
	std::vector<SAddr::Width> addrs; 
	for (auto t = g.loc_begin(); t != g.loc_end(); t++) {
		addrs.push_back(t->first.get());
	}
	std::sort(addrs.begin(), addrs.end());
	for (auto &a : addrs) {
		for (auto sIt = g.co_begin(a); sIt != g.co_end(a); ++sIt) {
			hash_combine(hash, sIt->getThread());			
			hash_combine(hash, sIt->getIndex());			
		}
	}
	return hash;
}

bool ExecutionGraphPtrEqual::operator()(const ExecutionGraph *lhs, const ExecutionGraph *rhs) const {
	auto &g = *lhs;
	auto &j = *rhs;
	if (g.getNumThreads() != j.getNumThreads())
		return false;
	for (auto tid = 0; tid < g.getNumThreads(); tid++) {
		if (g.getThreadSize(tid) != j.getThreadSize(tid))
			return false;
		for (auto index = 0; index < g.getThreadSize(tid); index++) {
			auto *eLab1 = g.getEventLabel({tid, index});
			auto *eLab2 = j.getEventLabel({tid, index});
			if (eLab1->getKind() != eLab2->getKind() || eLab1->getOrdering() != eLab2->getOrdering())
				return false;
			
			if (auto *rLab1 = llvm::dyn_cast<ReadLabel>(eLab1)) {
				auto *rLab2 = llvm::dyn_cast<ReadLabel>(eLab2);

				if (rLab1->getAddr() != rLab2->getAddr())
					return false;

				if (rLab1->getRf() && rLab2->getRf()) {
					if (rLab1->getRf()->getPos() != rLab2->getRf()->getPos())
						return false;
				} else if (rLab1->getRf() && !rLab2->getRf()) {
					return false;
				} else if (!rLab1->getRf() && rLab2->getRf()) {
					return false;
				}
			}
			else if (auto *wLab1 = llvm::dyn_cast<WriteLabel>(eLab1)) {
				auto *wLab2 = llvm::dyn_cast<WriteLabel>(eLab2);

				if (wLab1->getAddr() != wLab2->getAddr())
					return false;
				if (wLab1->getVal() != wLab2->getVal())
					return false;
			}
		}
	}
	for (auto &[addr, sList1] : g.getCoherence()) {
		if (!j.getCoherence().contains(addr))
			return false;
		auto &sList2 = j.getCoherence().at(addr);
		if (sList1.size() != sList2.size())
			return false;
		
		auto it1 = sList1.begin();
		auto it2 = sList2.begin();
		while (it1 != sList1.end() && it2 != sList2.end()) {
			if (it1->getPos() != it2->getPos())
				return false;
			it1++;
			it2++;
		}
