...
public:
	...
	std::vector<const llvm::GenericValue *>
	getTrackedCoherenceLocs() override;

	/* Returns the range of all the possible (i.e., not violating coherence
	 * places a store can be inserted without inserting it */
	std::pair<int, int>
	getPossiblePlacings(const llvm::GenericValue *addr, Event store, bool isRMW) override;
	 ...
	 * coherence */
	std::vector<Event>
	getCoherentRevisits(const WriteLabel *wLab, std::vector<Event> &&ls) override;
	...
	// TODO: refactor this?
	std::vector<Event>
	getPrefixRestricted(const WriteLabel *wLab, const VectorClock &view) const override;

	/* Returns the offset for a particular store */
	int getStoreOffset(const llvm::GenericValue *addr, Event store) const override;
