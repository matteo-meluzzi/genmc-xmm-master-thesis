	  ...
	  shouldHalt(false)
{
	...
	if (!conf->outputDotFile.empty()) {
		outputDotGraphFileStream = { std::ofstream(conf->outputDotFile) };
	}
}
...
GenMCDriver::handleThreadJoin(std::unique_ptr<ThreadJoinLabel> lab)
{
	...
	if (isExecutionDrivenByGraph(&*lab)) {
		auto *jLab = llvm::dyn_cast<ThreadJoinLabel>(g.getEventLabel(lab->getPos()));
		if (!llvm::isa<ThreadFinishLabel>(g.getLastThreadLabel(jLab->getChildId()))) {
			getEE()->block();
			return {};
		}
...
void GenMCDriver::mootExecutionIfFullyBlocked(Event pos)
{
	...
	if (auto *rLab = llvm::dyn_cast_or_null<ReadLabel>(lab))
		if (!rLab->isRevisitable() || !rLab->wasAddedMax() || g.getKind() != GraphKind::GenMC)
			moot();
	return;
}
