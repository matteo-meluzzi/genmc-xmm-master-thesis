...
void GenMCDriver::handleFinishedExecution()
{
	...
	/* Ignore the execution if there are unfulfilled promises
	 * (or all promises have been certified but with consistency violations) */
	// TODO: WKMODriver --- wait for patch for statistics?
//	if (!promises.empty()) {
//		promises.clear();
//		++exploredUnfulfilled;
//		return;
//	}
	...
	if (userConf->printExecGraphs && !isDup && !userConf->persevere)
		printGraph(); /* Delay printing if persevere is enabled */
	if (userConf->prettyPrintExecGraphs && !isDup)
		prettyPrintGraph();
...
std::unique_ptr<FRevItem> GenMCDriver::createForwardRevisit(Event r, Event w)
{
	return LLVM_MAKE_UNIQUE<FRevItem>(r, w);
}

std::unique_ptr<BRevItem> GenMCDriver::createBackwardRevisit(Event r, Event w)
{
	auto &g = getGraph();
	const EventLabel *revLab = g.getEventLabel(r);
	const EventLabel *sLab = g.getEventLabel(w);
	BUG_ON(!revLab || !sLab);
	...
	return LLVM_MAKE_UNIQUE<BRevItem>(r, w, std::move(writePrefix), std::move(moPlacings));
}

std::unique_ptr<MOItem> GenMCDriver::createMORevisit(Event p, int moPos)
{
	return LLVM_MAKE_UNIQUE<MOItem>(p, moPos);
}

bool GenMCDriver::isDuplicateRevisit(const BRevItem *bri)
{
	auto& g = getGraph();
	...
	if (!revisitSet[rLab->getStamp()].contains(writePrefixPos, bri->getMOPlacingsNoRel())) {
		revisitSet[rLab->getStamp()].add(writePrefixPos, bri->getMOPlacingsNoRel());
		return false;
	}
	return true;
}
...
RevisitSet &GenMCDriver::getRevisitSet(unsigned int stamp)
{
	return revisitSet[stamp];
}
...
bool GenMCDriver::isMoot() const
{
	return isMootExecution;
}
...
void GenMCDriver::checkForLoadBufferingPattern()
{
    const auto& g = getGraph();
    ...
    for (auto i = 0u; i < g.getNumThreads(); i++) {
        for (auto j = 0u; j < g.getThreadSize(i); j++) {
            auto wE = Event(i, j);
            if (auto *wLab = llvm::dyn_cast<WriteLabel>(g.getEventLabel(wE))) {
                const llvm::Type* wType = wLab->getType();
                const llvm::GenericValue* wAddr = wLab->getAddr();
                const auto& wVal = wLab->getVal();
                const auto wOrd = wLab->getOrdering();

                const auto& porf = wLab->getPorfView();
                const auto& hb = wLab->getHbView();

                for (auto m = 0u; m < g.getNumThreads(); m++) {
                    for (auto n = 0u; n < g.getThreadSize(m); n++) {
                        auto rE = Event(m, n);
                        if (!porf.contains(rE) || hb.contains(rE)) {
                            continue;
                        }
                        if (auto *rLab = llvm::dyn_cast<ReadLabel>(g.getEventLabel(rE))) {
                            const llvm::Type* rType = rLab->getType();
                            const llvm::GenericValue* rAddr = rLab->getAddr();
                            const auto rVal = getWriteValue(rLab->getRf(), rAddr, rType);
                            const auto rOrd = rLab->getOrdering();

                            if (wAddr == rAddr && wType == rType
                                && !getEE()->compareValues(wType, wVal, rVal)
                                && (wOrd == llvm::AtomicOrdering::Monotonic || rOrd == llvm::AtomicOrdering::Monotonic)
                                && !checkForFaiRfPath(rLab, wLab)
                            ) {
                                llvm::dbgs() << "pattern found" << "\n";
                                llvm::dbgs() << "racy accesses " << rE << " " << wE << "\n";
                                printGraph();
                                goto exit_loop;
                            }
                        }
...
bool GenMCDriver::checkForFaiRfPath(const ReadLabel* rLab, const WriteLabel* wLab)
{
    const auto& g = getGraph();

    auto* rFaiLab = llvm::dyn_cast<FaiReadLabel>(rLab);
    if (!rFaiLab) {
        return false;
    }
    auto* wFaiLab = llvm::dyn_cast<FaiWriteLabel>(wLab);
    while (wFaiLab) {
        auto* wrFaiLab = llvm::dyn_cast<FaiReadLabel>(g.getPreviousLabel(wFaiLab));
        if (!wrFaiLab) {
            return false;
        }
        if (rFaiLab == wrFaiLab) {
        ...
        auto wE = wrFaiLab->getRf();
        if (wE.isBottom() || wE.isInitializer()) {
            return false;
        }
        wFaiLab = llvm::dyn_cast<FaiWriteLabel>(g.getEventLabel(wE));
    }
    return false;
}
...
bool GenMCDriver::doPreConsChecks()
{
	return true;
}
...
bool GenMCDriver::isConsistent(ProgramPoint p)
{
	...
	return isConsistent(checkFull);
}

bool GenMCDriver::isConsistent(bool checkFull)
{
	if (!doPreConsChecks())
		return false;
...
std::vector<Event> GenMCDriver::properlyOrderStores(const ReadLabel *rLab, std::vector<Event> &stores)
{
	const auto &g = getGraph();

	llvm::Interpreter::InstAttr attr = llvm::Interpreter::IA_None;
	auto cmpVal = llvm::GenericValue();
	if (llvm::isa<FaiReadLabel>(rLab)) {
		attr = llvm::Interpreter::IA_Fai;
	}
	if (auto *casLab = llvm::dyn_cast<CasReadLabel>(rLab)) {
		attr = casLab->isLock() ? llvm::Interpreter::IA_Lock : llvm::Interpreter::IA_Cas;
		cmpVal = casLab->getExpected();
	}

	return properlyOrderStores(attr, rLab->getType(), rLab->getAddr(), cmpVal, stores);
}
...
{
	// TODO: it's a bit strange, but `isConsistent` first calls `shouldCheckCons` already
	if (shouldCheckCons(ProgramPoint::step)) {
		return isConsistent(ProgramPoint::step);
	}
	return true;
}

std::pair<int, int> GenMCDriver::getCoherentPlacings(const llvm::GenericValue *addr, Event store, bool isRMW)
{
	return getGraph().getCoherentPlacings(addr, store, isRMW);
}
		       ...
		       llvm::AtomicRMWInst::BinOp op)
{
	...
	for (auto it = validStores.begin() + 1; it != validStores.end(); ++it)
		addToWorklist(createForwardRevisit(lab->getPos(), *it));
...

{
	...
	auto placesRange =
		getCoherentPlacings(addr, pos, attr != llvm::Interpreter::IA_None &&
		                    attr != llvm::Interpreter::IA_Unlock);
	...
	// TODO: early exit here in case of equal write (WKMODriver)
	...
	for (auto it = locMO.begin() + begO; it != locMO.begin() + endO; ++it) {
		...
		/* Push the stack item */
		if (!inRecoveryMode()) {
			addToWorklist(createMORevisit(lab->getPos(), std::distance(locMO.begin(), it)));
		}
	}
...
{
	return calcRevisits(sLab, getRevisitLoads(sLab));
}

bool GenMCDriver::calcRevisits(const WriteLabel *sLab, std::vector<Event> &&loads)
{
	const auto &g = getGraph();
	...
	for (auto &l : loads) {
		...
		std::unique_ptr<BRevItem> bri =	createBackwardRevisit(rLab->getPos(), sLab->getPos());
...
//	if (!repairGraph(item.get())) {
//		return false;
//	}
	/* Restore the prefix, if this is a backward revisit */
	if (auto *bri = llvm::dyn_cast<BRevItem>(item.get()))
...
bool GenMCDriver::repairGraph(WorkItem *wi)
{
//	const EventLabel *lab = getGraph().getEventLabel(wi->getPos());
//	/* Restore the prefix, if this is a backward revisit */
//	if (auto *bri = llvm::dyn_cast<BRevItem>(wi))
//		restorePrefix(lab, bri->getPrefixRel(), bri->getMOPlacingsRel());
	return true;
}
			  ...
			  std::string functionName)
{
	...
	if (!lib->hasFunctionalRfs()) {
		...
		for (auto it = validStores.begin() + 1; it != validStores.end(); ++it)
			// TODO: replace with `createForwardRevisit`?
			   ...
			   void *transInode /* = NULL */)
{
	...
	auto placesRange = getCoherentPlacings(addr, pos, false);
	auto &begO = placesRange.first;
	auto &endO = placesRange.second;
	BUG_ON(begO != endO);
...
//				llvm::dbgs() << " (" << rLab->isRevisitable() << ")";
			} else if (auto *wLab = llvm::dyn_cast<WriteLabel>(lab)) {
				auto name = EE->getVarName(wLab->getAddr());
				auto moPos = cc->getStoreOffset(wLab->getAddr(), wLab->getPos());
				executeWLPrint(wLab, name, moPos);
			} else {
				llvm::dbgs() << *lab;
			}
...
//			llvm::dbgs() << "\t\t";
//			llvm::dbgs() << "#" << lab->getStamp() << " ";
//			llvm::dbgs() << lab->getPorfView() << "\t";
//			llvm::dbgs() << "\n";
		}
	}
	llvm::dbgs() << "\n";
