...
clXMM("XMM", llvm::cl::cat(clGeneral), llvm::cl::init(false),
      llvm::cl::desc("Enable XMM extension"));
...
static llvm::cl::opt<std::string>
clOutputDotGraphFile("dot-file", llvm::cl::init(""), llvm::cl::value_desc("file"),
			llvm::cl::cat(clGeneral),
			llvm::cl::desc("Write output graphs to DOT file format"));
...
static llvm::cl::opt<bool>
clRealTimeOutput("real-time-output", llvm::cl::cat(clDebugging),
			llvm::cl::desc("Print output in real-time"));

llvm::cl::opt<VerbosityLevel>
clVLevel(llvm::cl::cat(clDebugging), llvm::cl::init(VerbosityLevel::Warning),
	 ...
	 llvm::cl::values(
		 ...
		 clEnumValN(VerbosityLevel::Warning, "v2", "Print warnings (default)"),
...
void Config::checkConfigOptions() const
{
	...
	if (clXMM) {
		clDisableRaceDetection = true;
		clDisableInstructionCaching = true;
	}
...
void Config::saveConfigOptions()
{
	...
	outputDotFile = clOutputDotGraphFile;
	model = clModelType;
	...
	vLevel = clVLevel;
	realTimeOutput = clRealTimeOutput;
#ifdef ENABLE_GENMC_DEBUG
