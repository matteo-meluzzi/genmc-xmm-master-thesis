...
public:
	...
	/* Returns the locations at which coherence is tracked */
	virtual std::vector<const llvm::GenericValue *>
	getTrackedCoherenceLocs() = 0;
	...
	getPossiblePlacings(const llvm::GenericValue *addr, Event store, bool isRMW) = 0;
	...
	/* Returns all the reads from "ls" that "wLab" can revisit
	 * without violating coherence */
	virtual std::vector<Event>
	getCoherentRevisits(const WriteLabel *wLab, std::vector<Event> &&ls) = 0;
	...
	/* For the given store returns the coherence-earlier writes contained in the view.
	 * Valid prefix should contain at least the initialization event.
	 * Empty prefix means indicates an error or that the
	 * coherence calculator does not support this feature */
	// TODO: refactor this?
	virtual std::vector<Event>
	getPrefixRestricted(const WriteLabel *wLab, const VectorClock &view) const = 0;

	/* Returns the offset in coherence order for a particular store.
	 * Returns negative number if the offset is not defined
	 * (e.g. in case of Init event or because calculator does not track it) */
	 // TODO: rename/refactor?
	 virtual int
	 getStoreOffset(const llvm::GenericValue *addr, Event store) const = 0;
