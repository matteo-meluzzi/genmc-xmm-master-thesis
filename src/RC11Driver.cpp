...
View RC11Driver::calcBasicReadHBView(const ReadLabel *lab) const
{
	const auto &g = getGraph();

	View hb = calcBasicHbView(lab->getPos());

	if (lab->isAtLeastAcquire() && !lab->getRf().isBottom()) {
...
View RC11Driver::calcBasicReadPorfView(const ReadLabel *lab) const
{
	const auto &g = getGraph();
...
{
	View hb = calcBasicReadHBView(lab);
	View porf = calcBasicReadPorfView(lab);
...
{
	auto& g = getGraph();
	return g.getCoherentRevisits(sLab, g.getRevisitable(sLab));
}
