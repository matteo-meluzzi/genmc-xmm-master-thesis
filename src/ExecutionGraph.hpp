...
public:
	...
	GraphKind getKind() const {
		return kind;
	}

	void setKind(GraphKind k) {
		kind = k;
	}

	const LocMap &getCoherence() const {
		return coherence;
	}
...
protected:
	...
	GraphKind kind = GenMC;

	/* Dbg: Size of graphs which triggers a warning */
	unsigned int warnOnGraphSize = UINT_MAX;
};

struct ExecutionGraphPtrHasher {
	template <class T>
	inline void hash_combine(std::size_t& seed, const T& v)	const;

    std::size_t operator()(const ExecutionGraph *graph) const noexcept;
};

struct ExecutionGraphPtrEqual
{
    bool operator()(const ExecutionGraph *lhs, const ExecutionGraph *rhs) const;
};
