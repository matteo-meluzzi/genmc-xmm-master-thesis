...
public:
	...
	/* Checks that event belongs to the graph */
	bool checkInGraph(Event e) const;
	...
	EventLabel *getPreviousLabel(const EventLabel *lab);
	...
	EventLabel *getNextLabel(const EventLabel *lab);
	...
	/* Returns the first event/label in the thread tid */
	...
	/* Returns the last acquire before upperLimit in the latter's thread.
	 * If it's not a fence, then it has to be at location addr */
	Event getLastThreadAcquireAtLoc(Event upperLimit,
					const llvm::GenericValue *addr) const;
	...
	/* Checks that rLab can be revisited by sLab */
	bool isRevisitable(const ReadLabel *rLab, const WriteLabel *sLab) const;

	/* Returns a list of loads that can be revisited */
	virtual std::vector<Event> getRevisitable(const WriteLabel *sLab) const;
	virtual std::vector<Event> getRevisitable(const llvm::GenericValue *addr, const View &before) const;
	...
	/* Creates a copy of WriteLabel keeping only the readers satisfying property F */
	template <typename F>
	std::unique_ptr<WriteLabel> cloneWriteLabelAndKeepReaders(const WriteLabel *wLab, F cond) const {
		auto cpLab = std::unique_ptr<WriteLabel>(wLab->clone());
		cpLab->removeReader([&] (Event r) { return !cond(r); });
		return cpLab;
	}
	...
	/* Checks that event belongs to some po-rf cycle. */
	bool isOnPorfCycle(const EventLabel *lab) const;
	...
	std::vector<Event> getCoherentStores(const llvm::GenericValue *addr,
					     Event pos);
	std::pair<int, int> getCoherentPlacings(const llvm::GenericValue *addr, Event pos, bool isRMW);
	std::vector<Event> getCoherentRevisits(const WriteLabel *wLab, std::vector<Event> &&ls);
	...
	/* Saves a part of the given prefix that is not before rLab.
	...
	/* TODO: discuss with Michalis (refactoring) */
	virtual std::vector<std::unique_ptr<EventLabel> >
	getPrefixLabelsNotBefore(const VectorClock &prefix, const ReadLabel *rLab) const;
