...
std::vector<const llvm::GenericValue *> MOCoherenceCalculator::getTrackedCoherenceLocs()
{
...
std::pair<int, int>
MOCoherenceCalculator::getPossiblePlacings(const llvm::GenericValue *addr, Event store, bool isRMW)
{
...
int MOCoherenceCalculator::getStoreOffset(const llvm::GenericValue *addr, Event store) const
{
	BUG_ON(mo_.count(addr) == 0);

	if (store == Event::getInitializer())
		return -1;
...
std::vector<Event>
MOCoherenceCalculator::getCoherentRevisits(const WriteLabel *sLab, std::vector<Event> &&ls)
{
	const auto &g = getGraph();
...
std::vector<Event> MOCoherenceCalculator::getPrefixRestricted(const WriteLabel *wLab, const VectorClock &view) const
{
	const auto &g = getGraph();

	std::vector<Event> prefix = { Event::getInitializer() };

	auto &locMO = mo_.at(wLab->getAddr());
	auto prmIt = std::find(locMO.begin(), locMO.end(), wLab->getPos());
	BUG_ON(prmIt == locMO.end());

	for (auto it = locMO.begin(); it != prmIt; ++it) {
		if (!view.contains(*it))
			continue;
		prefix.push_back(*it);
	}
